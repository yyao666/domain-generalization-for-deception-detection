...
matrix_labels = y.expand(-1,bs) == y_transpose.expand(bs,-1)
print("\n\n",matrix_labels.float())

print(loss(m(dot_product),matrix_labels.float()))
...
class ContrastiveLoss(nn.Module):
    def __init__(self, ):
        ...
        self.scale = 10.0 # to make the sigmoid function more effective
        self.sigmoid = nn.Sigmoid()
        self.bce = nn.BCELoss()
    
    def process_logits(self, Features):
        f = nn.functional.normalize(Features, dim=-1)
        dot_product = f @ f.T # value varies between -1 to 1
        return self.sigmoid(self.scale * dot_product) # squashes value between 0 to 1
    
    def process_labels(self, Labels, bs):
        y = Labels.unsqueeze(1)   
        y_transpose = y.T
        y = y.expand(-1,bs)    
        y_transpose = y_transpose.expand(bs,-1)   # expand 
        matrix_labels = y.expand(-1,bs) == y_transpose.expand(bs,-1)   
        return matrix_labels.float()    

    def forward(self, features, labels):
        bs = features.shape[0]
        dot_product = self.process_logits(features)
        matrix_labels = self.process_labels(labels, bs)
        return self.bce(dot_product, matrix_labels)     # BCE to improve / drives the same of positive pairs


# loss = ContrastiveLoss()
# X = torch.rand(8,256)
# Y = torch.tensor([0,1,1,0,1,1,0,1],dtype=torch.int64)
# print(loss(X,Y))
# exit()
...
def train_one_epoch(train_data_loader, model, optimizer, loss1, loss2, beta):
    deception_loss = []
    contrastive_loss = []
    sum_correct_pred = 0
    total_samples = 0
    ...
    for i,(spec,deception_labels) in enumerate(train_data_loader):
        ...
        _contrastive_loss = loss2(contrastive_projections,deception_labels)
        _loss = _deception_loss + beta * _contrastive_loss              
        deception_loss.append(_deception_loss.item())
        contrastive_loss.append(_contrastive_loss.item())     
    ...
    return deception_loss, contrastive_loss, acc
...
for betas in beta_rates:
    ...
    for tr,te in protocols: 
        ...
        for epoch in range(num_epochs):
            ...
            # print('\n\tEpoch...........',epoch+1) 
            # print("\tTrain deception loss = {}; \tTrain contrastive loss = {}; \tTrain accuracy = {}".format(train_deception_loss, train_contrastive_loss, train_acc))
